      ...
      <el-table-column label="操作" width="280" align="center">
        <template slot-scope="scope">
          <el-popconfirm
              ...
              icon-color="red"
              title="您确定选择吗？"
              @confirm="selectCourse(scope.row.id)"
          >
          <el-button type="primary" slot="reference" style="margin-right: 5px">选课</el-button>
          </el-popconfirm>
...
export default {
  ...
  methods: {
    selectCourse(courseId) {
      this.request.post('http://localhost:9090/course/studentCourse/' + courseId + "/" + this.user.id).then(res => {
        if (res.code === '200') {
    ...
    load() {
      ...
      this.request.get("http://localhost:9090/user/role/ROLE_TEACHER").then(res => {
        this.teachers = res.data
      })
    },
    changeEnable(row) {
      this.request.post("http://localhost:9090/course/update", row).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("http://localhost:9090/course/" + id).then(res => {
        if (res.code === '200') {
